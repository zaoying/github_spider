...
function get(path) {
    ...
    return callback => https.get(options, resp => {
        let json = ''
...
function getRepos(author, repo) {
    let path = REPOS + '/' + author + '/' + repo;
    get(path).call(this, repo => {
...
function getRelease(author, repo) {
    let path = REPOS + '/' + author + '/' + repo + RELEASE;
    get(path).call(this, releases => {
...
function getLanguage(author, repo) {
    let path = REPOS + '/' + author + '/' + repo + LANGUAGE;
    get(path).call(this, console.log)
}

getRepos("BurntSushi", "toml")

getRelease("BurntSushi", "toml")

// getLanguage("BurntSushi", "toml")